...
runcmd(struct cmd *cmd)
{
  ...
  char * env[] = {"/bin","/usr/bin","/usr/local/bin/"};
  int pipefd[2];
  pid_t cpid;
  ...
  case ' ':
    ...
    // Your code here ...
    //int ret = execvpe(ecmd->argv[0],ecmd->argv,env);
    int ret = execvp(ecmd->argv[0],ecmd->argv);
    if(ret == -1){
	fprintf(stderr,"file:%s,err:%s\n",ecmd->argv[0],strerror(errno));
    }
    break;
  ...
  case '|':
	...
	//left end early than right,put it in child thread should be ok
	runcmd(pcmd->left);
    }else{//parent
